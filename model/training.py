...
class Trainer(object):
    ...
    def render_visdata(self, data, resolution, it, out_render_path):
        (img, dpt, camera_mat, scale_mat, img_idx, mask) = self.process_data_dict(data)
        h, w = resolution
        if self.pose_param_net:
    ...
    def process_data_dict(self, data):
        ...
        return (img, dpt, camera_mat, scale_mat, img_idx, mask_img)
    def process_data_reference(self, data):
        ...
        ref_masks = data.get('img.ref_masks').to(device)
        return ( ref_imgs, ref_dpts, ref_idxs, ref_masks)
    ...
    def compute_loss(self, data, eval_mode=False, it=None, epoch=None, scheduling_start=None, out_render_path=None):
        ...
        (img,  depth_input, camera_mat_gt, scale_mat, img_idx, mask_img) = self.process_data_dict(data)   
        if use_ref_imgs:
            (ref_img, depth_ref, ref_idx, ref_masks) = self.process_data_reference(data)
        ...
        # Sample pixels
        # NB!!! check if it is necessary to process directly as torch
        mask_img = mask_img.cpu().numpy()
        t_mask = np.reshape(mask_img, -1).astype('float32')
        t_mask[t_mask == 0] = 0.2 # I still want a bit of background
